// Pair features only exist for the genx engine; gen1-3 builds get a stub main
// so the workspace still compiles (same pattern as selfplay's eval_api shim).
#[cfg(any(feature = "gen1", feature = "gen2", feature = "gen3"))]
fn main() {
    eprintln!("eval-pair-features requires a gen4+ build");
    std::process::exit(1);
}
